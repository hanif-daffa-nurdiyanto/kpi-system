...
use App\Filament\Admin\Resources\KpiDailyEntryResource\Pages;
                ...
                Section::make('KPI Metrics')
                    ->schema([
                        ...
                        Repeater::make('kpi_metrics')
                                ...
                                Grid::make(3)
                                    ->schema([
                                        Select::make('metric_id')
                                            ->label('KPI Metric')
                                            ->options(KpiMetric::where('is_active', true)->get()->pluck('name', 'id'))
                                            ->searchable()
                                            ->required()
                                            ->reactive()
                            ...
                            ->itemLabel(fn(array $state): ?string => isset($state['metric_id']) ? KpiMetric::find($state['metric_id'])?->name : null)
            ...
            ->filters([
                ...
                SelectFilter::make('department')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query->when(
                            $data['value'],
                            fn(Builder $query, $departmentId): Builder => $query->whereHas('user.employee', function ($q) use ($departmentId) {
                    ...
                    ->indicateUsing(fn($data) => $data['value']
                        ? 'Department: ' . Department::find($data['value'])->name
                        : null),
                    ...
                    ->query(function (Builder $query, array $data) {
                        if (!empty($data['options_date'])) {
                            switch ($data['options_date']) {
                                ...
                                case 'last_month':
                                    return $query->whereMonth('entry_date', Carbon::now()->subMonth()->month)
                    ...
                    ->indicateUsing(function (array $data) {
                        if (!empty($data['options_date'])) {
                            switch ($data['options_date']) {
                                ...
                                case 'last_week':
                                    return 'Entry: Last Week';
                                case 'last_month':
                                    return 'Entry: Last Month';
                            }
                        }
                        return null;
                    }),
                ...
                Filter::make('submitted')
                    ...
                    ->toggle()
                    ->indicateUsing(fn(array $data) => $data['isActive'] ? 'Submitted Entries Only' : null),

                Filter::make('pending_approval')
                    ->label('Pending Approval')
                    ->query(fn(Builder $query): Builder => $query->where('status', 'submitted'))
                    ->toggle()
                    ->indicateUsing(fn(array $data) => $data['isActive'] ? 'Pending Approval Only' : null),
            ])
            ->actions([
                Tables\Actions\ActionGroup::make([
                    ...
                    Tables\Actions\Action::make('approve')
                        ...
                        ->color('success')
                        ->requiresConfirmation()
                        ->visible(fn($record) => in_array($record->status, ['submitted']))
                        ->action(function ($record) {
                            $record->update(['status' => 'approved']);

                            Notification::make()
                                ->title('KPI Entry Approved')
                                ->body("Your KPI entry has been approved by an administrator.")
                    ...
                    Tables\Actions\Action::make('reject')
                        ...
                        ->action(function ($record) {
                            ...
                            Notification::make()
                                ...
                                ->body("Your KPI entry has been rejected by an administrator.")
